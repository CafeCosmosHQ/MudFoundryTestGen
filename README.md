...
We wanted to work in foundry as it can be a bit easy to lose control using `pnpm mud test` so we built this to generate a base test that handles deploying and registering systems and tables. This is also useful to get a better understanding of how mud works for someone who is more foundry/solidity native.

## Args 
`-o` or `--output` specify output path of the base test
`-nb` or `--no-build` skips `mnpm mud build` which skips building the contracts. Useful if you want to generate the test and already have a `.mud/local/systems.json` that matches the current contracts. Hence if you haven't added or changed any tables or added or removed any systems
